...
def write_result_to_csv(result, commit_hash):
    ...
    with CSV_PATH.open("a", newline="") as f:
        ...
        if is_new:
            writer.writerow([
                "Date", "Commit", "Script", "CPU Time (s)", "Calls", "Prim Calls",
                "Avg Time/Call (ms)", "Acquire Time Total (s)", "Selection Time Total (s)", "Transform Time Total (s)","Observations Extracted", "Time Per Observation Extracted (s)"
            ])
        ...
        writer.writerow([
            ...
            (result["acquire_time"] + result["selection_time"] + result["transform_time"]) / (result["observations_extracted"]-1)
        ])
