...
def sheets_from_workbook(source: Any, selectable: Selectable, workbook: Workbook, custom_time_formats: Dict[str, str]) -> List[XlsxSelectable]:
        ...
        for y, row in enumerate(worksheet.iter_rows()):
            ...
            for x, opycell in enumerate(row):
                if opycell.is_date and opycell.internal_value is not None:
                    strformat_pattern = xlsx_time_formats().get(opycell.number_format, None)
                    if strformat_pattern is None:
                        strformat_pattern = custom_time_formats.get(opycell.number_format, None)
                        if strformat_pattern is None:
                            raise UnknownExcelTimeError(missing_time_format_message(opycell.number_format))
