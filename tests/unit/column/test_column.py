...
def test_apply_results_are_cached_so_same_modification_is_not_resolved_multiple_times():
    ...
    column = Column(data.excel_ref('1').finds_observations_directly(down), apply=lambda x: x)
...
def test_apply_results_are_unique_to_a_given_column_context():
    ...
    column1 = Column(data.excel_ref('1').finds_observations_directly(down), apply=lambda x: "foo "+x)
