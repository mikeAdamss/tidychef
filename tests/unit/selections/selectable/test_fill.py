...
def test_expand(selectable_simple1: Selectable):
    ...
    s = selectable_simple1.excel_ref("E2").fill(
        DOWN
    ) - selectable_simple1.excel_ref("E4").fill(DOWN)
    assert len(s.cells) == 2

    s = selectable_simple1.excel_ref("W75").fill(RIGHT).fill(UP)
    assert len(s.cells) == 222

    s = selectable_simple1.excel_ref("D4").fill(
        LEFT
    ) - selectable_simple1.excel_ref("Z4").fill(LEFT)
